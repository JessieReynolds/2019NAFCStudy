...
```{r}
lmort.m1<- drm(TotalDeath/Initial ~ Treatment, weights=TotalDeath, data=lmortdata, fct=LL.2(), type="binomial")
summary(lmort.m1)
```
...
BMD at 10%:
```{r}
bmd(lmort.m1, 0.1, def=c("excess"), interval = c("delta"), display = TRUE)
```
